...
class PixelEraser {
  ...
  private getOrCreateBlock(tileKey: string, blockKey: string, blockX: number, blockY: number): fogMap.Block | null {
...
describe("Performance Test - DelPixel", () => {
  test("should measure performance of delPixel operation", () => {
    ...
    for (let lat = centerLat - areaSize; lat <= centerLat + areaSize; lat += lineSpacing) {
      map = map.addLine(centerLng - areaSize, lat, centerLng + areaSize, lat);
    }
    ...
    const erasePaths = [
      ...
      {
        x0: centerLng - areaSize * 0.8,
        y0: centerLat + areaSize * 0.2,
        x1: centerLng + areaSize * 0.8,
        y1: centerLat + areaSize * 0.2,
      },
      {
        x0: centerLng - areaSize * 0.8,
        y0: centerLat - areaSize * 0.2,
      ...
      {
        x0: centerLng - areaSize * 0.7,
        y0: centerLat + areaSize * 0.4,
        x1: centerLng + areaSize * 0.7,
        y1: centerLat + areaSize * 0.4,
      },
      {
        x0: centerLng - areaSize * 0.7,
        y0: centerLat - areaSize * 0.4,
      ...
      {
        x0: centerLng + areaSize * 0.3,
        y0: centerLat - areaSize * 0.8,
        x1: centerLng + areaSize * 0.3,
        y1: centerLat + areaSize * 0.8,
      },
      {
        x0: centerLng - areaSize * 0.3,
        y0: centerLat - areaSize * 0.8,
      ...
      // Diagonal strokes
      {
        x0: centerLng - areaSize * 0.6,
        y0: centerLat - areaSize * 0.6,
        x1: centerLng + areaSize * 0.6,
        y1: centerLat + areaSize * 0.6,
      },
      {
        x0: centerLng - areaSize * 0.6,
        y0: centerLat + areaSize * 0.6,
      ...
      {
        x0: centerLng - areaSize * 0.5,
        y0: centerLat + areaSize * 0.7,
        x1: centerLng + areaSize * 0.5,
        y1: centerLat + areaSize * 0.7,
      },
      {
        x0: centerLng - areaSize * 0.5,
        y0: centerLat - areaSize * 0.7,
    ...
    process.stdout.write(
      `  → Remaining Tiles: ${afterEraseMetrics.tileCnt}, Blocks: ${afterEraseMetrics.blockCnt}\n\n`
    );
    ...
    process.stdout.write(
      `  Removal Rate: ${(
        ((afterDrawMetrics.blockCnt - afterEraseMetrics.blockCnt) / afterDrawMetrics.blockCnt) *
        100
      ).toFixed(1)}%\n\n`
    );
