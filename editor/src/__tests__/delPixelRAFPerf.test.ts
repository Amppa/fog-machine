...
describe("RAF Batch Processing Performance Test", () => {
  ...
  beforeEach(() => {
    ...
    mockContext = {
      ...
      map: {
        ...
        project: (_lngLat: unknown) => ({ x: 100, y: 100 }),
        unproject: (_point: unknown) => ({ lng: 120, lat: 24 }),
        flyTo: jest.fn(),
        addControl: jest.fn(),
  ...
  test("should reduce UI update frequency with RAF batching", async () => {
    ...
    process.stdout.write(
      `Mouse Move (${mouseMoveCount} events): ${moveUpdateCount} update(s) in ${moveDuration.toFixed(2)} ms\n`
    );
    ...
    process.stdout.write(
      `✅ RAF batching is working! Reduced from ${mouseMoveCount} to ${moveUpdateCount} updates\n\n`
    );
  });
  ...
  test("should compare performance: with RAF vs without RAF simulation", async () => {
    ...
    process.stdout.write(
      `✅ RAF reduces UI updates by ~${((1 - withRAFUpdates / mouseMoveCount) * 100).toFixed(0)}%\n\n`
    );
  });
});
