    ...
    <>
      <div className="p-4 bg-gray-50">
        <span className="flex items-center">
          <span className="text-sm font-medium text-gray-900">{t("map-type")}</span>
        </span>
        <div className="w-full pt-4">
          <Tab.Group
            onChange={(index) => {
              const style = mapStyles[index];
              mapController.setMapStyle(style as "standard" | "satellite" | "hybrid" | "none");
            }}
            defaultIndex={mapStyles.indexOf(mapController.getMapStyle())}
          >
            <Tab.List className="flex p-1 space-x-1 bg-gray-300 rounded-xl">
              {[t("map-type-standard"), t("map-type-satellite"), t("map-type-hybrid"), t("map-type-none")].map(
                (category) => (
                  <Tab
                    key={category}
                    className={({ selected }) => {
      ...
      <div className="p-4 bg-gray-50">
        <span className="flex items-center">
          <span className="text-sm font-medium text-gray-900">{t("fog-concentration")}</span>
        </span>
        <div className="w-full pt-4">
          <Tab.Group
            onChange={(index) => {
              const fogConcentration = fogConcentrations[index];
              mapController.setFogConcentration(fogConcentration as "low" | "medium" | "high");
            }}
            defaultIndex={fogConcentrations.indexOf(mapController.getFogConcentration())}
          >
            <Tab.List className="flex p-1 space-x-1 bg-gray-300 rounded-xl">
              {[t("fog-concentration-low"), t("fog-concentration-medium"), t("fog-concentration-high")].map(
                (category) => (
                  <Tab
                    key={category}
                    className={({ selected }) => {
      ...
      <div className="p-4 bg-gray-50">
        <span className="flex items-center">
          <span className="text-sm font-medium text-gray-900">{t("map-projection")}</span>
        </span>
        <div className="w-full pt-4">
          <Tab.Group
            onChange={(index) => {
              const mapProjection = mapProjections[index];
              mapController.setMapProjection(mapProjection as "globe" | "mercator");
            }}
            defaultIndex={mapProjections.indexOf(mapController.getMapProjection())}
          >
            <Tab.List className="flex p-1 space-x-1 bg-gray-300 rounded-xl">
              {[t("map-projection-globe"), t("map-projection-mercator")].map((category) => (
                <Tab
                  key={category}
                  className={({ selected }) => {
                    return (
      ...
      {exportProgress && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-6 rounded-lg shadow-xl">
            <h3 className="text-lg font-medium text-gray-900 mb-2">{t("exporting")}</h3>
